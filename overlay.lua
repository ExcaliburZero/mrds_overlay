...
function display_monster_changes(current_time, monster)
    ...
    elseif changes_previous_monster["stress"] == -1 then
        changes_previous_monster = monster
    elseif changes_previous_monster["stress"] ~= monster["stress"] or changes_previous_monster["fatigue"] ~= monster["fatigue"] or changes_previous_monster["lifespan"] ~= monster["lifespan"] then
    ...
    if changes_remaining_duration > 0 then
        ...
        if changes_previous_monster["fatigue"] ~= monster["fatigue"] then
            if monster["fatigue"] > changes_previous_monster["fatigue"] then
                gui.text(1, 172, string.format("         +%d", monster["fatigue"] - changes_previous_monster["fatigue"]), "green")
            else
                gui.text(1, 172, string.format("         %d", monster["fatigue"] - changes_previous_monster["fatigue"]), "red")
            end
        end

        if changes_previous_monster["lifespan"] ~= monster["lifespan"] then
            if monster["lifespan"] > changes_previous_monster["lifespan"] then
                gui.text(1, 172, string.format("                +%d", monster["lifespan"] - changes_previous_monster["lifespan"]), "green")
            else
                gui.text(1, 172, string.format("                %d", monster["lifespan"] - changes_previous_monster["lifespan"]), "red")
            end
        end
    end
